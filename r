    ...
    private fun FmRecord.fraudRecords(
        ...
        return if (processType == 11 && !monitoringProcessFraudSchemas.isNullOrEmpty()) {
            val monitoringFrauds = monitoringProcessFraudSchemas.map { scheme ->
                val fraudScheme = fraudReasonsCorp.firstOrNull { scheme.fraudSchemeId == it.id } ?: throw FrontException("id=$id fraudSchemeId=${scheme.fraudSchemeId}")
...
}  private fun suspicionFraudSchemeId(
    fraudSchemeCode: String,
    fraudSchemeCorps: List<FraudReasonsCorp>
): Int = fraudSchemeCorps.associate { it.key to it.id }[fraudSchemeCode]
    ?: throw FrontException(NOT_FOUND_FRAUD_SCHEME_ID)    {"message":" id=619001 fraudSchemeId=1"} 
